...
async def handle_media_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если есть media_group_id - собираем все части
    if media_group_id:
        ...
        group = pending_media[media_group_id]
        ...
        # Ждем 3 секунды для сбора всех частей
        ...
        if caption.strip():
            ...
            media_type = "видео + фото" if session.get("video") else f"{len(session['photos'])} фото"
        ...
        # Удаляем группу
        del pending_media[media_group_id]
...
async def handle_photo_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in ["title_auto", "title_custom", "title_cancel"]:
...
async def handle_music_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "no_music":
        ...
        await query.edit_message_text(f"⏳ Скачиваю музыку '{audio_name}'...")
...
async def handle_all_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если есть media_group_id - это медиагруппа
    if media_group_id:
...
async def main():
    ...
    # Callback'и
    app.add_handler(CallbackQueryHandler(handle_photo_callback, pattern="^(photo_post|photo_video|title_auto|title_custom|title_cancel|process_media)$"))
