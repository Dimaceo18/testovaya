...
CHANNEL_LINK = os.getenv("CHANNEL_LINK", "https://t.me/your_channel")  # Ссылка на закрытый канал
...
# Храним информацию о пользователях, подавших заявку
user_requests = {}
...
async def back_to_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Пересоздаем стартовое сообщение
    keyboard = [
        [InlineKeyboardButton("📝 Подать заявку", callback_data="apply")],
        [InlineKeyboardButton("❓ Что это?", callback_data="info")]
    ]
...
async def admin_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        user_id = int(text.replace('/approve_', '').strip())
    except ValueError:
...
async def admin_reject(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        user_id = int(text.replace('/reject_', '').strip())
    except ValueError:
